...
generate-keys:
	...
	$(PHP_CONT) setfacl -R -m u:www-data:rX -m u:"$(whoami)":rwX config/jwt
	$(PHP_CONT) setfacl -dR -m u:www-data:rX -m u:"$(whoami)":rwX config/jwt
...
wait-services:
	...
	$(MAKE) wait-service PORT_NUMBER=${CADDY_HTTP_PORT}; \
	$(MAKE) wait-service PORT_NUMBER=${CADDY_HTTPS_PORT}; \
...
check-ports:
	...
	$(MAKE) check-port PORT_NUMBER=${CADDY_HTTP_PORT}; \
	$(MAKE) check-port PORT_NUMBER=${CADDY_HTTPS_PORT}; \
...
## -- First time install ---------------------------------------------------------------
.PHONY: install
install: check-requirements build up wait-services db-reset generate-keys
